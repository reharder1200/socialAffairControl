...
<%@ attribute name="codeName" type="java.lang.String" required="false" description="编码名称（Name）"%>
<%@ attribute name="codeValue" type="java.lang.String" required="false" description="编码名称（Name）"%>
...
<div class="input-append">
	...
	<input id="${id}Code" name="${codeName}" class="${cssClass}" type="hidden" value="${codeValue}"/>
		...
		top.$.jBox.open("iframe:${ctx}/tag/treeselect?url="+encodeURIComponent("${url}")+"&module=${module}&checked=${checked}&extId=${extId}&isAll=${isAll}", "选择${title}", 300, 420, {
			ajaxData:{selectIds: $("#${id}Id").val()},buttons:{"确定":"ok", ${allowClear?"\"清除\":\"clear\", ":""}"关闭":true}, submit:function(v, h, f){
				if (v=="ok"){
					...
					$("#${id}Code").val(codes.join(",").replace(/u_/ig,""));
